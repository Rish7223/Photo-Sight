...
const useLikePhoto = (photo, userId) => {
  ...
  const likePhoto = async () => {
    try {
      ...
      if (res) {
        const removeLikeRes = await collectionRef.doc(photoId).update({
          likes: firebase.firestore.FieldValue.arrayRemove(userId)
        });
        !removeLikeRes && setIsLiked(false);
        useFetchPhotosLikes();
      } else {
        const addLikeRes = await collectionRef.doc(photoId).update({
          likes: firebase.firestore.FieldValue.arrayUnion(userId)
        });
        !addLikeRes && setIsLiked(true);
        useFetchPhotosLikes();
      }
    } catch (error) {
      if (error.message) {
